...
const EmployeeDashboard = () => {
    ...
    useEffect(() => {
        ...
        // Promise.all([fetchStatus(), fetchLeaves(), fetchPayslip()]).then(...).finally(() => setIsLoading(false))
        setIsLoading(false);
    }, []);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <button className="flex flex-col items-center justify-center p-6 bg-blue-50 hover:bg-blue-100/80 rounded-xl transition-colors group">
                                ...
                                <>
                                    <div>
                                        <div className="flex justify-between text-sm mb-1.5">
                                            ...
                                            <span className="text-gray-900 font-semibold">{leaveBalances.casual.used} / {leaveBalances.casual.total}</span>
                                        </div>
                                        <div className="w-full bg-gray-100 rounded-full h-2">
                                            <div className="bg-blue-600 h-2 rounded-full" style={{ width: `${(leaveBalances.casual.used / leaveBalances.casual.total)*100}%` }}></div>
                                        </div>
                                    </div>
                                    <div>
                                        <div className="flex justify-between text-sm mb-1.5">
                                            <span className="text-gray-600 font-medium">Sick Leave</span>
                                            <span className="text-gray-900 font-semibold">{leaveBalances.sick.used} / {leaveBalances.sick.total}</span>
                                        </div>
                                        <div className="w-full bg-gray-100 rounded-full h-2">
                                            <div className="bg-emerald-500 h-2 rounded-full" style={{ width: `${(leaveBalances.sick.used / leaveBalances.sick.total)*100}%` }}></div>
                                        </div>
                                    </div>
                                    <div>
                                        <div className="flex justify-between text-sm mb-1.5">
                                            <span className="text-gray-600 font-medium">Vacation</span>
                                            <span className="text-gray-900 font-semibold">{leaveBalances.vacation.used} / {leaveBalances.vacation.total}</span>
                                        </div>
                                        <div className="w-full bg-gray-100 rounded-full h-2">
                                            <div className="bg-purple-500 h-2 rounded-full" style={{ width: `${(leaveBalances.vacation.used / leaveBalances.vacation.total)*100}%` }}></div>
                                        </div>
                                    </div>
                                </>
                            ) : (
